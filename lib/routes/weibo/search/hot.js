...
module.exports = async (ctx) => {
    ...
    const listData = data.cards[0].card_group.map((item) => {
    ...
    let listHtml = '';
    listData.forEach((hotItem) => {
        listHtml = `${listHtml}
        <li style="list-style: none;">
            <image width="24"  style="vertical-align: bottom;" src="${hotItem.pic}"/>
            <a href="${hotItem.link}">${hotItem.title}</a>
            ${hotItem.icon && `<image  style="vertical-align: bottom;" width="24" src="${hotItem.icon}"/>`}
        </li>`;
    });
